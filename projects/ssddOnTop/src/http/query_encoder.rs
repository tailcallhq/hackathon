...
impl QueryEncoder {
    pub fn encode(&self, key: &str, raw_value: Option<ValueString>) -> String {
        if let Some(value) = raw_value {
            match &value {
                ValueString::Value(val) => self.encode_const_value(key, val.serde()),
    ...
    fn encode_const_value(&self, key: &str, value: &serde_json::Value) -> String {
        match self {
...
pub fn convert_value(value: &serde_json::Value) -> Option<String> {
    match value {
...
mod tests {
    ...
    fn test_encode_comma_separated_arg() {
        ...
        let binding = crate::value::Value::new(values);
        let arg_raw_value = Some(ValueString::Value(Cow::Borrowed(&binding)));
    ...
    fn test_encode_single_value_comma_separated() {
        let key = "single";
        let values = Value::Array(vec![Value::String("value".to_string())]);
        let strategy = QueryEncoder::CommaSeparated;
