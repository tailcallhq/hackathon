...
use crate::config::Type1;
use crate::json::{JsonLike, JsonObjectLike};
    ...
    pub fn new(union_name: &str, union_types: &[(&str, &Type1)]) -> Result<Self> {
        ...
        // Validation to ensure no two types have the same set of fields.
        {
            ...
            if !duplicates.is_empty() {
                return Err(anyhow!("Union have equal types"));
            }
        }
...
mod tests {
    ...
    fn test_single_distinct_field_optional() {
        let foo = Type1::default().fields(vec![("foo", Field::default())]);
        let bar = Type1::default().fields(vec![("bar", Field::default())]);
    ...
    fn test_fields_protobuf_oneof() {
        let var_var = Type1::default().fields(vec![("usual", Field::default())]);
        let var0_var = Type1::default().fields(vec![
            ("usual", Field::default()),
            (
                "payload",
                Field {
    ...
    fn test_additional_types() {
        let type_a = Type1::default().fields(vec![
        ...
        let type_c = Type1::default().fields(vec![
            ("uniqueC1", Field::default()),
            ("uniqueC2", Field::default()),
        ]);
        let type_d = Type1::default().fields(vec![
    ...
    fn validation_number_of_types() {
        let types: Vec<_> = (0..136)
            .map(|i| (i.to_string(), Type1::default()))
            .collect();
