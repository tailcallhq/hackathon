...
pub fn expand_directive_definition(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl macros_common::directive_definition::DirectiveDefinition for #struct_identifier {
            fn directive_definition(generated_types: &mut std::collections::HashSet<String>) -> Vec<async_graphql::parser::types::TypeSystemDefinition> {
                let schemars = macros_common::into_schemars::<Self>();
                let attr = macros_common::directive_definition::Attrs {
                    name: stringify!(#struct_identifier),
                    repeatable: #is_repeatable,
                    locations: vec![#(#locations),*],
                    is_lowercase_name: #is_lowercase_name
                };
                macros_common::directive_definition::into_directive_definition(schemars, attr, generated_types)
            }
        }
    };
...
pub fn expand_input_definition(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl macros_common::input_definition::InputDefinition for #struct_identifier {
            fn input_definition() -> async_graphql::parser::types::TypeSystemDefinition {
                let schemars = macros_common::into_schemars::<Self>();
                macros_common::input_definition::into_input_definition(schemars.schema, stringify!(#struct_identifier))
            }
        }
    };
